...
//------------------------------------------------------------------------------
// Original Simpler Primitives Reverted
//------------------------------------------------------------------------------
...
static Mesh makeSphere(int lat = 24, int lon = 24, float radius = 0.85f) {
	Mesh m;
...
static Mesh makeTorus(int rings = 32, int sides = 16, float R = 0.6f, float r = 0.25f) {
	Mesh m;
...
//------------------------------------------------------------------------------
// Procedural Fracture Tree Path Generator
//------------------------------------------------------------------------------
...
static Mesh buildOverlayMeshFromPaths(const std::vector<std::vector<glm::vec3>>& paths, int shapeType, float width, float depth) {
	...
	for (const auto& path : paths) {
		...
		for (size_t i = 0; i < path.size(); ++i) {
			...
			leftSide.push_back(p - t * (width * 0.5f) + n * 0.004f);
			rightSide.push_back(p + t * (width * 0.5f) + n * 0.004f);
...
//------------------------------------------------------------------------------
// Recursive Triangle Splitting & Adaptive Refinement Carver Engine
//------------------------------------------------------------------------------
static void subdivideAndCarveTriangle(const glm::vec3& v0, const glm::vec3& v1, const glm::vec3& v2,
									  const glm::vec3& n0, const glm::vec3& n1, const glm::vec3& n2,
									  const std::vector<std::vector<glm::vec3>>& paths,
									  float radius, float depth, int level, int shapeType, Mesh& outMesh) {
	...
	for (const auto& path : paths) {
		for (size_t i = 0; i < path.size() - 1; ++i) {
			...
			glm::vec3 b = path[i+1];
			glm::vec3 ab = b - a;
			glm::vec3 ap = center - a;
			float t = glm::clamp(glm::dot(ap, ab) / (glm::dot(ab, ab) + 1e-6f), 0.0f, 1.0f);
			glm::vec3 closest = a + t * ab;
			float dst = glm::length(center - closest);
			if (dst < minDst) minDst = dst;
		}
	}

	// 1. If triangle intersects fracture pathway area, recursively subdivide to increase local triangle count[cite: 5]
	if (minDst < radius * 1.5f && level < 3) {
		glm::vec3 m01 = (v0 + v1) * 0.5f;
		glm::vec3 m12 = (v1 + v2) * 0.5f;
		glm::vec3 m20 = (v2 + v0) * 0.5f;

		// Snap back to base surface boundaries before displacement calculation loops
		m01 = projectToSurface(m01, shapeType);
		m12 = projectToSurface(m12, shapeType);
		m20 = projectToSurface(m20, shapeType);

		glm::vec3 n01 = glm::normalize(n0 + n1);
		glm::vec3 n12 = glm::normalize(n1 + n2);
		glm::vec3 n20 = glm::normalize(n2 + n0);

		subdivideAndCarveTriangle(v0, m01, m20, n0, n01, n20, paths, radius, depth, level + 1, shapeType, outMesh);
		subdivideAndCarveTriangle(m01, v1, m12, n01, n1, n12, paths, radius, depth, level + 1, shapeType, outMesh);
		subdivideAndCarveTriangle(m20, m12, v2, n20, n12, n2, paths, radius, depth, level + 1, shapeType, outMesh);
		subdivideAndCarveTriangle(m01, m12, m20, n01, n12, n20, paths, radius, depth, level + 1, shapeType, outMesh);
	} 
	// 2. Base case: Apply profile displacement indentation to localized high-density vertices
	else {
		glm::vec3 finalV[3] = {v0, v1, v2};
		for (int k = 0; k < 3; ++k) {
			float vDst = 1e9f;
			for (const auto& path : paths) {
				for (size_t i = 0; i < path.size() - 1; ++i) {
					glm::vec3 a = path[i]; glm::vec3 b = path[i+1];
					glm::vec3 ab = b - a; glm::vec3 ap = finalV[k] - a;
					float t = glm::clamp(glm::dot(ap, ab) / (glm::dot(ab, ab) + 1e-6f), 0.0f, 1.0f);
					glm::vec3 closest = a + t * ab;
					float d = glm::length(finalV[k] - closest);
					if (d < vDst) vDst = d;
				}
			}
			if (vDst < radius) {
				float falloff = 1.0f - (vDst / radius);
				glm::vec3 vNorm = (shapeType == 0) ? n0 : glm::normalize(finalV[k]);
				finalV[k] -= vNorm * (depth * falloff); // Carves volume inwards
			}
		}
		outMesh.verts.insert(outMesh.verts.end(), {finalV[0], finalV[1], finalV[2]});
		outMesh.normals.insert(outMesh.normals.end(), {n0, n1, n2});
	}
}

static void carveMeshWithAdaptiveSubdivision(Mesh& active, const Mesh& pristine, const std::vector<std::vector<glm::vec3>>& paths, float width, float depth, int shapeType) {
	active.verts.clear();
	active.normals.clear();
	float radius = width * 0.5f;

	// Loop through baseline layout primitives and adaptively fractionate where appropriate[cite: 5]
	for (size_t i = 0; i < pristine.verts.size(); i += 3) {
		subdivideAndCarveTriangle(
			pristine.verts[i],   pristine.verts[i+1],   pristine.verts[i+2],
			pristine.normals[i], pristine.normals[i+1], pristine.normals[i+2],
			paths, radius, depth, 0, shapeType, active
		);
	}

	// Recompute normal framework over fresh incisions
	for (size_t i = 0; i < active.verts.size(); i += 3) {
		glm::vec3 n = glm::normalize(glm::cross(active.verts[i+1] - active.verts[i], active.verts[i+2] - active.verts[i]));
...
struct App {
	...
	GPUMesh activeHostMesh;
	GPUMesh previewCrackMesh;
	...
	void updatePreviewModel() {
		...
		for (int i = 0; i < mainArms; ++i) {
			...
			generateBranch(calculatedPaths, activeStrikePoint, armDir, 22, shapeIndex, crackJitter);
		}
	...
	void resetHostGeometry() {
		activeHostMesh.clear();
		activeHostMesh = uploadMesh(pristineMeshes[shapeIndex]);
		activeVertexCount = pristineMeshes[shapeIndex].verts.size();
	}

	void bakeVolumeCarve() {
		activeHostMesh.clear();
		Mesh carved;
		carveMeshWithAdaptiveSubdivision(carved, pristineMeshes[shapeIndex], calculatedPaths, crackWidth, crackDepth, shapeIndex);
		activeHostMesh = uploadMesh(carved);
		activeVertexCount = carved.verts.size();
	}
};
...
static void frame(void* arg) {
	...
	ImGui::Spacing(); ImGui::Separator();
	ImGui::Text("Active Vertex Profile Metrics:");
	...
	// 1. Draw primitive target base mesh
	glUniform3fv(glGetUniformLocation(app.program, "diffuseColor"), 1, glm::value_ptr(app.diffuseColor));
	if (app.activeHostMesh.count > 0) {
		glBindVertexArray(app.activeHostMesh.vao);
		if (app.wireframe) glDrawElements(GL_LINES, app.activeHostMesh.wireCount, GL_UNSIGNED_INT, (void*)0);
		else glDrawArrays(GL_TRIANGLES, 0, app.activeHostMesh.count);
	}

	// 2. Draw live line track preview overlay blueprint[cite: 5]
	if (app.previewCrackMesh.count > 0) {
		glUniform3fv(glGetUniformLocation(app.program, "diffuseColor"), 1, glm::value_ptr(app.previewColor));
		glBindVertexArray(app.previewCrackMesh.vao);
		if (app.wireframe) glDrawElements(GL_LINES, app.previewCrackMesh.wireCount, GL_UNSIGNED_INT, (void*)0);
		else glDrawArrays(GL_TRIANGLES, 0, app.previewCrackMesh.count);
	}
...
int main() {
	...
	static App app;
	app.window = glfwCreateWindow(900, 700, "Starburst Adaptive Carver", nullptr, nullptr);
	...
	// Cache internal standard simple primitives
	app.pristineMeshes[0] = makeCube();
